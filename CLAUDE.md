...
- **Seed partial-insert trap**: The seed inserts the dealer row first, then units. If the unit insert fails mid-batch (e.g., a non-integer value in an integer column), the dealer row is already committed — and on retry, `onConflictDoNothing` silently skips the dealer, leaving it with zero units. Symptom: "Skipped (already exists)" for a dealer that actually has no inventory. Fix: delete the orphan dealer row, then re-seed. Most common cause is dirty data in the JSON (floats where integers are expected, etc.) — validate the JSON before re-seeding.
